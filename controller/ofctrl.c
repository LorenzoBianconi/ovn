...
static void
ofctrl_meter_bands_sync(struct ovn_extend_table_info *entry,
                        const struct sbrec_meter_table *meter_table,
                        struct ovs_list *msgs)
{
    const struct sbrec_meter *sb_meter;
    SBREC_METER_TABLE_FOR_EACH (sb_meter, meter_table) {
        if (!strcmp(entry->name, sb_meter->name)) {
            break;
        }
    }
    ...
    if (sb_meter) {
        ...
        ofctrl_meter_bands_update(sb_meter, entry, msgs);
    } else {
        /* OFPMC13_DELETE */
        ofctrl_meter_bands_erase(entry, msgs);
    }
}

static void
add_meter(struct ovn_extend_table_info *entry,
          const struct sbrec_meter_table *meter_table,
          struct ovs_list *msgs)
{
    const struct sbrec_meter *sb_meter;
    ...
    if (!sb_meter) {
        static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 1);
        VLOG_ERR_RL(&rl, "could not find meter named \"%s\"", entry->name);
        return;
    }
