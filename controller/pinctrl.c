    ...
    struct ofpbuf *continuation)
{
    ...
    unsigned char *filename = NULL, *boot_filname_opt_ptr = NULL;
    ...
    while (ptr < ptr_end) {
        ...
        case 300: /* DHCP_OPT_FILENAME */
          filename = ptr + sizeof *in_dhcp_opt;
          filename_len = in_dhcp_opt->len;
          break;
    ...
    if (!ipxe_req && filename_len > 0 && boot_filename_len > 0) {
        memcpy(&boot_filname_opt_ptr[2], filename, filename_len);
        boot_filname_opt_ptr[1] = filename_len;
        opt_len += filename_len - boot_filename_len;
    }
