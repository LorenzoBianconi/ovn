                          ...
                          struct ofpbuf *userdata)
{
    ...
    *dhcp_hdr = DHCPV6_MSG_TYPE_REQUEST;
    ...
    put_load(md->flow.regs[MFF_LOG_INPORT - MFF_REG0], MFF_LOG_OUTPORT, 0,
             32, &ofpacts);
...
pinctrl_find_prefixd_state(const struct flow *ip_flow, unsigned aid)
{
    ...
    SHASH_FOR_EACH (iter, &ipv6_prefixd) {
        struct ipv6_prefixd_state *pfd = iter->data;
        if (IN6_ARE_ADDR_EQUAL(&pfd->ipv6_addr, &ip_flow->ipv6_src) &&
            eth_addr_equals(pfd->ea, ip_flow->dl_src) &&
            pfd->aid == aid) {
            return pfd;
        }
    }
...
compose_prefixd_solicit(struct dp_packet *b,
                        ...
                        const struct eth_addr cmac, int aid)
{
    eth_compose(b, eth_dst, eth_src, ETH_TYPE_IPV6, IPV6_HEADER_LEN);

    int len = UDP_HEADER_LEN + 4 +
