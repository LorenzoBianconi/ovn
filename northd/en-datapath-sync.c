...
datapath_sync_sb_datapath_binding(struct engine_node *node, void *data)
{
    ...
    SBREC_DATAPATH_BINDING_TABLE_FOR_EACH_TRACKED (sb_dp, sb_dp_table) {
        ...
        if (sbrec_datapath_binding_is_new(sb_dp) && !sdp->update_sb_dp) {
            /* There is a new datapath binding, but we already have
             * a synced datapath with a permanent pointer to the
             * SB datapath binding. This indicates that somebody
             * has added a duplicate datapath to the SB database.
