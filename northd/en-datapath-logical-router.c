...
static void
gather_external_ids(const struct nbrec_logical_router *nbr, struct smap *external_ids)
{
                                                  ...
                                                  void *data)
{
    ...
    NBREC_LOGICAL_ROUTER_TABLE_FOR_EACH_TRACKED (nbr, nb_lr_table) {
        ...
        if (is_new) {
            ...
            gather_external_ids(nbr, &external_ids);
            int64_t old_requested_tunnel_key = udp->requested_tunnel_key;
            int64_t new_requested_tunnel_key = smap_get_int(
                &nbr->options, "requested-tnl-key", 0);
            if (old_requested_tunnel_key != new_requested_tunnel_key) {
                udp->requested_tunnel_key = new_requested_tunnel_key;
                hmapx_add(&map->updated, udp);
                result = EN_HANDLED_UPDATED;
            } else if (!smap_equal(&external_ids, &udp->external_ids)) {
