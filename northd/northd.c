                    ...
                    struct ovn_datapath_binding *dp)
{
    ...
    od->tunnel_key = dp->sb->tunnel_key;
                                     ...
                                     struct ovn_datapath *od)
{
    ...
    sbrec_ip_multicast_set_datapath(sb, od->dp->sb);
...
{
    char *mp_name = ovn_mirror_port_name(ovn_datapath_name(op->od->dp->sb),
                                         nb_mirror->sink);
                       ...
                       struct hmapx *mirror_attached_ports)
{
    ...
    if (op && (op->od || op->nbsp || op->nbrp)) {
        ...
        return NULL;
    } else if (op && (!op->sb || op->sb->datapath == od->dp->sb)) {
        /*
               ...
               struct ovs_list *both_dbs, struct ovs_list *nb_only)
{
    ...
    if (crp && crp->sb && crp->sb->datapath == op->od->dp->sb) {
...
{
    sbrec_port_binding_set_datapath(op->sb, op->od->dp->sb);
    if (op->nbrp) {
                         ...
                         struct northd_data *nd)
{
    ...
    NBREC_LOGICAL_SWITCH_TABLE_FOR_EACH_TRACKED (new_ls,
                                             ni->nbrec_logical_switch_table) {
        ...
        struct ovn_synced_logical_switch *lsw =
            ovn_synced_logical_switch_find(ni->synced_lses,
                                           &new_ls->header_.uuid);
        if (!lsw) {
            goto fail;
        }
    ...
    const struct ovn_datapaths *lr_datapaths)
{
    ...
    SBREC_DATAPATH_BINDING_TABLE_FOR_EACH_TRACKED (sbrec_dp, sbrec_dp_table) {
        ...
        struct ovn_datapath *od;
        if (!strcmp(sbrec_dp->type, "logical-switch")) {
            od = ovn_datapath_find_(&ls_datapaths->datapaths, sbrec_dp->nb_uuid);
        } else {
            od = ovn_datapath_find_(&lr_datapaths->datapaths, sbrec_dp->nb_uuid);
        }

        if (!sbrec_datapath_binding_is_deleted(sbrec_dp) && !od) {
            VLOG_WARN_RL(&rl, "Datapath "UUID_FMT" is present in the "
                         "Southbound DB with mismatching "
                         "external-ids:logical-switch/router "UUID_FMT,
                         UUID_ARGS(&sbrec_dp->header_.uuid),
                         UUID_ARGS(sbrec_dp->nb_uuid));
            return false;
        }
        ...
        if (sbrec_datapath_binding_is_new(sbrec_dp)) {
            if (!od) {
                VLOG_WARN_RL(&rl, "A datapath-binding for %s is created but "
                             "the %s is not found.", name, sbrec_dp->type);
                return false;
            }
            od->dp->sb = sbrec_dp;
            ...
            if (od && od->dp->sb == sbrec_dp) {
                VLOG_WARN_RL(&rl, "A datapath-binding for %s is deleted but "
                             "the %s still exists.", name, sbrec_dp->type);
                return false;
            }
        } else {
            /* sbrec_dp is updated.*/
                 ...
                 struct hmap *ls_datapaths)
{
    ...
    HMAP_FOR_EACH (od, key_node, ls_datapaths) {
        ...
        for (size_t i = 0; i < od->nbs->n_dns_records; i++) {
            ...
            dns_info->sbs[dns_info->n_sbs - 1] = od->dp->sb;
        }
    }
               ...
               struct hmap *ls_datapaths)
{
    ...
    HMAP_FOR_EACH (od, key_node, ls_datapaths) {
        ...
        const struct sbrec_ip_multicast *ip_mcast =
            ip_mcast_lookup(sbrec_ip_mcast_by_dp, od->dp->sb);
    ...
    struct hmap *lr_ports)
{
    ...
    SBREC_STATIC_MAC_BINDING_TABLE_FOR_EACH_SAFE (sb_smb,
        ...
        if (!op || !op->nbrp || !op->od || !op->od->dp->sb) {
            sbrec_static_mac_binding_delete(sb_smb);
        }
    }
        ...
        if (op && op->nbrp) {
            ...
            if (od && od->dp->sb) {
                ...
                if (!mb) {
                    ...
                    sbrec_static_mac_binding_set_datapath(mb, od->dp->sb);
                } else {
