...
add_route(struct lflow_table *lflows, struct ovn_datapath *od,
          ...
          const struct sset *bfd_ports, const struct ovsdb_idl_row *stage_hint,
          bool is_discard_route, int ofs, struct lflow_ref *lflow_ref)
{
                        ...
                        struct lflow_ref *lflow_ref)
{
    ...
    add_route(lflows, route_->is_discard_route ? od : out_port->od, out_port,
              ...
              bfd_ports, &route->header_, route_->is_discard_route, ofs,
              lflow_ref);
...
build_routable_flows_for_router_port(
    ...
    const struct sset *bfd_ports, struct lflow_table *lflows,
    struct ds *match, struct ds *actions)
{
                                         ...
                                         struct lswitch_flow_build_info *lsi)
{
    ...
    build_neigh_learning_flows_for_lrouter_port(op, lsi->lflows, &lsi->match,
                                                &lsi->actions, op->lflow_ref);
    build_ip_routing_flows_for_lrp(op, lsi->bfd_ports, lsi->lflows,
                                   op->lflow_ref);
