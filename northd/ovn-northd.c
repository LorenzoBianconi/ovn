...
static void
build_lrouter_flows_for_snat_lb(struct ovn_lb_vip *lb_vip,
                                struct ovn_northd_lb *lb,
                                ...
                                struct hmap *lflows)
{
    ...
    for (int i = 0; i < lb->n_nb_lr; i++) {
        ...
        if (od->l3redirect_port &&
            ...
            new_match = xasprintf("ct.new && %s && is_chassis_resident(%s)",
                                  ds_cstr(&match),
                                  od->l3redirect_port->json_key);
            est_match = xasprintf("ct.est && %s && is_chassis_resident(%s)",
                                  ds_cstr(&match),
                                  od->l3redirect_port->json_key);
        } else {
        ...
        if (snat_type == FORCE_SNAT || snat_type == SKIP_SNAT) {
            ...
            ovn_lflow_add_with_hint(lflows, od, S_ROUTER_IN_DNAT, prio,
                                    new_match, new_actions,
                                    &lb->nlb->header_);
            free(new_actions);
        } else {
            ovn_lflow_add_with_hint(lflows, od, S_ROUTER_IN_DNAT, prio,
                                     new_match, ds_cstr(&action),
                                     &lb->nlb->header_);
        }
                           ...
                           struct shash *meter_groups)
{
    ...
    for (size_t i = 0; i < lb->n_vips; i++) {
        ...
        build_lrouter_flows_for_snat_lb(lb_vip, lb, &lb->vips_nb[i],
                                        lflows);
    }
